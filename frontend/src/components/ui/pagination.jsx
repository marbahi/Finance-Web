...
const ROLL = { duration: 0.18, ease: EASE };
const STILL = { duration: 0 };

const slotFor = (digits) => Math.max(32, 18 + digits * 8);
const GAP = 4;
...
export function paginate(page, count, siblings, boundaries) {
...
export function usePagination({
  ...
  const clampTo = useCallback(
    (value) => Math.min(Math.max(1, value), Math.max(1, count)),
    [count],
  );
