...
import { Gear, User, Bell, Database, Download, Question, Trash } from '@phosphor-icons/react'
...
export default function Settings() {
  ...
  const sections = [
    { key: 'general', label: 'Umum', icon: Gear },
    { key: 'profile', label: 'Profil', icon: User },
    { key: 'notifications', label: 'Notifikasi', icon: Bell },
    { key: 'data', label: 'Data', icon: Database },
    { key: 'about', label: 'Tentang', icon: Question },
  ]
            ...
            return (
              <button key={s.key} onClick={() => setActiveSection(s.key)}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors ${
                  activeSection === s.key ? 'bg-gray-900 text-white font-medium' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-100'
                }`}>
                <Icon size={16} />
                {s.label}
              </button>
